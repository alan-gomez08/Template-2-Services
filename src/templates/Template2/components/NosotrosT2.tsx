  ...
  const features = [
    {
      ...
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="1.8" viewBox="0 0 24 24" style={{ color: paleta.colorAcento }}>
            ...
            {features.map((feature, index) => (
              <div key={index} className="flex flex-col items-center md:items-start text-center md:text-left gap-4 md:gap-3 w-full max-w-[360px] md:max-w-none lg:max-w-[284px]">
                <div 
                  ...
                  style={{ backgroundColor: `${paleta.colorAcento}0D`, borderColor: `${paleta.colorAcento}40` }}
                >
                  {feature.icon}
                </div>
