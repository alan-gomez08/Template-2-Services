...
export default function ColeccionT2({ data, paleta }: Props) {
          ...
          <div className="w-full max-w-[1320px] mx-auto px-4 lg:px-8">
            <div ref={sliderRef} className="flex gap-4 md:gap-6 justify-start items-stretch overflow-x-auto snap-x snap-mandatory scroll-smooth pb-8 hide-scrollbar w-full">
              {filteredItems.map((item) => (
                <article 
                  key={item.id} 
                  className="shrink-0 flex-none w-[280px] md:w-[calc(50%-12px)] lg:w-[calc(25%-18px)] snap-start rounded-[16px] p-4 flex flex-col gap-4 shadow-[0px_10px_30px_rgba(0,0,0,0.06)] hover:shadow-[0px_15px_40px_rgba(0,0,0,0.1)] transition-shadow"
                ...
                >
                  ...
                  <div className="flex flex-col gap-1 px-1 flex-grow">
                    ...
                    <h3 className="text-[18px] lg:text-[20px] font-bold font-['Manrope'] leading-tight truncate" style={{ color: paleta.textoPrimario }}>{item.title}</h3>
                    <div className="flex gap-1 mt-1">
                      {[...Array(5)].map((_, i) => (
                        <svg key={i} className="w-[14px] h-[14px]" style={{ color: paleta.colorPrimario }} fill="currentColor" viewBox="0 0 20 20"><path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" /></svg>
                      ))}
                    </div>
                  </div>
                  ...
                  <a 
                    ...
                    className="w-full py-3.5 mt-2 rounded-full flex justify-center items-center gap-2 text-[15px] font-semibold font-['Inter'] transition-colors active:scale-95 hover:opacity-90"
                    style={{ backgroundColor: paleta.textoPrimario, color: paleta.fondoCajas }}
                  >
